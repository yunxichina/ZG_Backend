...
import com.central.oauth2.common.token.*;
...
public class OAuth2Controller {
    ...
    @ApiOperation(value = "律师用户名密码获取token")
    @PostMapping(SecurityConstants.LAWYER_PASSWORD_LOGIN_PRO_URL)
    public void getLawyerTokenInfo(
        ...
        LawyerUsernameAuthenticationToken token = new LawyerUsernameAuthenticationToken(username, password);
        writerToken(request, response, token, "用户名或密码错误");
    }


    @ApiOperation(value = "客户用户名密码获取token")
    @PostMapping(SecurityConstants.CUSTOMER_PASSWORD_LOGIN_PRO_URL)
    public void getCustomerTokenInfo(
            @ApiParam(required = true, name = "username", value = "账号") String username,
            @ApiParam(required = true, name = "password", value = "密码") String password,
            HttpServletRequest request, HttpServletResponse response) throws IOException {
        CustomerUsernameAuthenticationToken token = new CustomerUsernameAuthenticationToken(username, password);
        writerToken(request, response, token, "用户名或密码错误");
    }
    ...
    public void getUserTokenInfo(
            ...
            @ApiParam(required = true, name = "password", value = "密码") String password,
            HttpServletRequest request, HttpServletResponse response) throws IOException {
        UsernameAuthenticationToken token = new UsernameAuthenticationToken(username, password);
        writerToken(request, response, token, "用户名或密码错误");
    }
