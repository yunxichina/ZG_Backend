...
import com.central.common.model.sysuser.SysMenu;
    ...
    public LoginAppUser getLoginAppUser(SysUser sysUser) {
        if (sysUser != null) {
            ...
            if (!CollectionUtils.isEmpty(sysRoles)) {
                ...
                List<SysMenu> menus = roleMenuMapper.findAllMenusByRoleIds(roleIds);

                if(!CollectionUtils.isEmpty(menus)){
                    //拼接所有的permissions
                    Set<Map<String,Object>> permissions = new HashSet<> ();

                        for (SysMenu menu:menus) {
                            //该写法效率低。需要改造
                            if(menu.getType() == CommonConstant.MENU){

                                Map<String,Object> per = new HashMap<>();

                                per.put("permissionId",menu.getKey());
                                per.put("permissionName",menu.getTitle());
                                per.put("parentId", menu.getParentId());
                                //添加actions
                                Set<Map<String, Object>> actions = getActions(menu.getId(), menus);
                                if(!CollectionUtils.isEmpty(actions)){
                                    per.put("actions", actions);
                                    per.put("actionEntitySet",actions);
                                }
                                permissions.add(per);
            ...
            log.info(loginAppUser.toString());

            return loginAppUser;
        }
        return null;
    }
     private Set<Map<String,Object>> getActions(Long menuId,List<SysMenu> menus){
         Set<Map<String,Object>> actions = new HashSet<> ();
        //根据当前menu 获取所有下属action
         for (SysMenu menu:menus) {
             if(menu.getType() == CommonConstant.PERMISSION&&menu.getParentId() == menuId){
                Map<String, Object> mper = new HashMap<> ();
                mper.put("action", menu.getKey());
                mper.put("describe",menu.getTitle());
                mper.put("defaultCheck", false);
                actions.add(mper);
             }
         }
         return actions;
